 ...
 * @author You-Chi Liu, Kevine Twagizihirwe, Dartmouth CS 10, Winter 22
...
public class CollisionGUI extends DrawingGUI {
	...
	private PointQuadtree<Blob> blobTree;       // tree that contains all blobs
	...
	private void findColliders() {
		...
		for (Blob blob : blobs) {
			if (blobTree == null) {
				blobTree = new PointQuadtree<Blob>(blob, 0, 0, width, height);
			}
			else  blobTree.insert(blob); // create the tree with each blob
		}

		for (Blob blob : blobs) {
			// use list inContact to keep track of the blobs that collided with each blob
			List<Blob> inContact = blobTree.findInCircle(blob.getX(), blob.getY(), 2 * blob.getR());
			if (inContact.size() >= 2) {
				colliders.addAll(inContact);
			}
		}
	}
