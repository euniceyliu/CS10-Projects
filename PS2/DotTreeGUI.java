 ...
 * @author Kevine Twagizihirwe, You-Chi Liu, Dartmouth CS 10, Winter 2022
...
public class DotTreeGUI extends DrawingGUI {
	...
	public void handleMousePress(int x, int y) {
		if (mode == 'a') {
			...
			if (tree == null) {
				...
				tree.insert(new Dot(x, y));} // ensures that tree is not null and adds a new dot at the pressed location
		}
		else if (mode == 'q') {
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		for (int i = 1; i <=4; i++){
			if (tree.hasChild(i)){
				drawTree(g,tree.getChild(i),1+level); // draw each node's children if it is not a leaf
		}
		}
//
	}
